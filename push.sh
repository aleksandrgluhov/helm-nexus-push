...
declare HELM3_VERSION
declare REPO
declare REPO_URL

HELM3_VERSION="$(helm version --client --short | grep "v3\.")"
REPO="${1}"
# Added whitespace to grep regex
# https://github.com/sonatype-nexus-community/helm-nexus-push/issues/28
REPO_URL="$(helm repo list | grep "^$REPO[[:space:]]" | awk '{print $2}')/"

if [[ -n $HELM3_VERSION ]]; then
    REPO_AUTH_FILE="$HOME/.config/helm/auth.$REPO"
else
    REPO_AUTH_FILE="$(helm home)/repository/auth.$REPO"
fi
...
case "$2" in
    login)
        if [[ -z "${NEXUS_USERNAME}" ]]; then
            read -r -p "Username: " NEXUS_USERNAME
        fi
        if [[ -z "${NEXUS_PASSWORD}" ]]; then
            read -r -s -p "Password: " NEXUS_PASSWORD
            echo
        fi
